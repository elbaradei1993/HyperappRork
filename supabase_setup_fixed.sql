...
-- Enable RLS for saved_places table
ALTER TABLE public.saved_places ENABLE ROW LEVEL SECURITY;

-- Create policies for saved_places table
CREATE POLICY "Enable read for own saved places" ON public.saved_places
    FOR SELECT USING (
        auth.uid() = user_id
    );

CREATE POLICY "Enable insert for authenticated users" ON public.saved_places
    FOR INSERT WITH CHECK (
        auth.uid() = user_id
    );

CREATE POLICY "Enable update for own saved places" ON public.saved_places
    FOR UPDATE USING (
        auth.uid() = user_id
    );

CREATE POLICY "Enable delete for own saved places" ON public.saved_places
    FOR DELETE USING (
        auth.uid() = user_id
    );

-- Create trigger for saved_places updated_at
